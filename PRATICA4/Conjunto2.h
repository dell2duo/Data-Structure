...
class Conjunto{
    ...
    public:
        ...
        const Conjunto<T> operator+(const Conjunto<T> &) const;
        const Conjunto<T> operator*(const Conjunto<T> &) const;
        const Conjunto<T> operator-(const Conjunto<T> &) const;
    private:
        T *elementos = NULL;
...
template <class T>
const Conjunto<T> Conjunto<T>::operator+(const Conjunto<T> &conj) const{
    Conjunto<T> resp(this->tam_array + conj.tam_array);

    resp.elementos = new T[resp.tam_array];
    for(int i=0;i<this->numelementos();i++){
        resp.insere(this->elementos[i]);
    }

    for(int i=0;i<conj.numelementos();i++){
        resp.insere(conj.elementos[i]);
    }
...
const Conjunto<T> Conjunto<T>::operator*(const Conjunto<T> &conj) const{
    Conjunto<T> resp;
    if(this->numelementos() < conj.numelementos()){
        resp.tam_array = this->numelementos();
        resp.elementos = new T[resp.tam_array];
        for(int i=0;i<this->numelementos();i++){
            if(conj.pertence(this->elementos[i])){
                resp.insere(this->elementos[i]);
            }
        }
    }
    else{
        resp.tam_array = conj.numelementos();
        resp.elementos = new T[resp.tam_array];
        for(int i=0;i<conj.numelementos();i++){
            if(this->pertence(conj.elementos[i])){
                resp.insere(conj.elementos[i]);
            }
        }
    }
...
template <class T>
const Conjunto<T> Conjunto<T>::operator-(const Conjunto<T> &conj) const{
    Conjunto<T> resp(this->tam_array);
