...
void MyList2<T>::insert(const T&elem, iterator whereIt) {
	...
	Node<T> *temp = new Node<T>(elem);
	Node<T> *aux = whereIt.ptr;
	whereIt--;

	aux->prev = temp;
	temp->next = aux;

	whereIt.ptr->next = temp;
	temp->prev = whereIt.ptr;
...
template<class T>
void MyList2<T>::recursiveReverse(Node<T> *temp){
	if(temp == NULL) return;
	Node<T> *aux = temp->prev;
	temp->prev = temp->next;
	temp->next = aux;
	recursiveReverse(temp->prev);
}
