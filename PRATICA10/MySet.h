...
private:
	...
	void Destroy(Node<T> *);
	Node<T> *insereNodo(Node<T> *, Node<T> *) const;
	//funcoes auxiliares...
...
template <class T>
void MySet<T>::Destroy(Node<T> *raiz){
	if(raiz == NULL) return;
	Destroy(raiz->left);
	Destroy(raiz->right);
	delete raiz;
...
template <class T>
Node<T> * MySet<T>::insereNodo(Node<T> *raizOther, Node<T> *prev) const{
	if(raizOther == NULL) return raizOther;

	Node<T> *raizThis = new Node<T>(raizOther->elem);

	raizThis->parent = prev;
	raizThis->left = insereNodo(raizOther->left, raizThis);
	raizThis->right = insereNodo(raizOther->right, raizThis);
...
MySet<T> & MySet<T>::operator=(const MySet<T>&other){
	...
	this->root = insereNodo(other.root, NULL);
