...
void MyHashMap<KEY,VALUE,HASH>::set(const  KEY &k, const VALUE &v) {
	...
	int hash = funcaoHash(k)%DEFAULT_TABLE_SIZE;
	typename MyList2<std::pair<KEY, VALUE>>::iterator it = table[hash].begin();
	while(it != table[hash].end()){
		if((*it).first == k) {(*it).second = v; return;}
		it++;
	}
	std::pair<KEY, VALUE> elem(k, v);
